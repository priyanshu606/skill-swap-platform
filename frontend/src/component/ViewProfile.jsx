  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white shadow-lg rounded-lg mt-10">
      <div className="flex items-center gap-6">
        <img
          src={`http://localhost:8009${user.profilePhoto}`}
          alt="Profile"
      ...
      <div className="mt-6">
        <h3 className="font-semibold text-lg text-indigo-700">Skills Offered</h3>
        <ul className="flex flex-wrap gap-2 mt-2">
          {user.skillsOffered.map((skill, index) => (
            <li key={index} className="bg-indigo-100 px-3 py-1 rounded-full text-sm">
              {skill}
            </li>
          ))}
        </ul>

        <h3 className="font-semibold text-lg text-pink-700 mt-4">Skills Wanted</h3>
        <ul className="flex flex-wrap gap-2 mt-2">
          {user.skillsWanted.map((skill, index) => (
            <li key={index} className="bg-pink-100 px-3 py-1 rounded-full text-sm">
              {skill}
            </li>
          ))}
        </ul>
